...
rules = do
    ...
    phony "lint" $ do
        ...
        let roots = map takeDirectory cabalFiles
        let srcs = map srcDirs cabals
        let paths = concat $ zipWith (\dir -> map (dir </>)) roots srcs
        putNormal $ "Linting: " ++ unwords paths
        cmd (Traced "hlint") (WithStdout True) "stack exec hlint --" paths
